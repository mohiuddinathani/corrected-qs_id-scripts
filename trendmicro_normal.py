# Trendmicro_normal.py (Final Production Version)
import os
import re
import logging
import psycopg2
...
logger = logging.getLogger("trendmicro_normalizer")
load_dotenv()
...
def safe_numeric(value):
    """Sanitizes a value to be a valid float or None."""
    if value is None:
        return None
    try:
        ...
        with psycopg2.connect(**DB_CONFIG) as conn:
            with conn.cursor() as cur:
                ...
                for staging_id, raw_data in tqdm(rows, desc=f"Parsing {VENDOR_NAME} Data"):
                    advisory_id = raw_data.get("zdi_id")
                    cve_id = raw_data.get("cve")
                    if not advisory_id or not cve_id: continue
                    ...
                    cves[cve_key] = (
                        vendor_id, cve_id,
                        None, # cwe_id
                        clean_text(raw_data.get("vulnerability_details")),
                        None, # severity
                    ...
                    cve_product_maps[cve_key] = (
                        vendor_id, cve_id, None, clean_text(raw_data.get("additional_details"))
                    )
...
'''
# Trendmicro_normal.py (Production Version)
import os
import re
import logging
import psycopg2
            ...
            with conn.cursor() as cur:
                ...
                if cves:
                    execute_values(cur, """
                        ...
                        ON CONFLICT (vendor_id, cve_id) DO UPDATE SET description=EXCLUDED.description, cvss_score=EXCLUDED.cvss_score, cvss_vector=EXCLUDED.cvss_vector;
                    """, list(cves.values()))
                if cve_product_maps:
                    execute_values(cur, """
