# spring_normal.py (Final Production Version)
import os
import re
import json
import logging
...
logger = logging.getLogger("spring_normalizer")
load_dotenv()
        ...
        with psycopg2.connect(**DB_CONFIG) as conn:
            with conn.cursor() as cur:
                ...
                for staging_id, raw_data in tqdm(rows, desc=f"Parsing {VENDOR_NAME} Data"):
                    primary_cve = raw_data.get("cve_id_from_detail") or raw_data.get("cve_id_from_list")
                    advisory_id = f"SPR-{raw_data['url'].split('/')[-1].upper()}" if primary_cve else raw_data.get("title")
                    
                    # 1. Advisory Record
                    advisories[advisory_id] = (
                        advisory_id, vendor_id, raw_data.get("title"),
                        raw_data.get("severity"), parse_date(raw_data.get("published_date")),
                        None, # latest_update_date
                        raw_data.get("url")
                    )
                    
                    cve_list = sorted(list(set(re.findall(r'CVE-\d{4}-\d{4,7}', str(raw_data)))))
                    ...
                    for cve_id in cve_list:
                        ...
                        cves[cve_key] = (
                            vendor_id, cve_id, 
                            None, # cwe_id
                            raw_data.get("description"), raw_data.get("severity"),
                            None, None, # cvss_score, cvss_vector
                            None, None, # initial/latest release dates
                            raw_data.get("references") or raw_data.get("url")
                        )
                        
                        # 3. Product Map
                        cve_product_maps[cve_key] = (
                            vendor_id, cve_id, None, raw_data.get("mitigation", "See advisory for details.")
                        )
...
'''
# spring_normal.py (Production Version)
import os
import re
import json
import logging
        ...
        with psycopg2.connect(**DB_CONFIG) as conn:
            with conn.cursor() as cur:
                ...
                for staging_id, raw_data in tqdm(rows, desc=f"Parsing {VENDOR_NAME} Data"):
                    ...
                    # Advisory Record - latest_update_date is NULL
                    advisories[advisory_id] = (
                        advisory_id, vendor_id, raw_data.get("title"),
                        raw_data.get("severity"), parse_date(raw_data.get("published_date")),
                        None, # latest_update_date
