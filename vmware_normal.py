# VMWare_normal.py (Final Production Version)
import os
import re
import logging
import json
...
logger = logging.getLogger("vmware_normalizer")
load_dotenv()
...
def find_cves_in_text(data_blob):
    return sorted(list(set(re.findall(r'CVE-\d{4}-\d{4,7}', json.dumps(data_blob), re.IGNORECASE))))
        ...
        with psycopg2.connect(**DB_CONFIG) as conn:
            with conn.cursor() as cur:
                ...
                for staging_id, raw_data in tqdm(rows, desc=f"Parsing {VENDOR_NAME} Data"):
                    summary = raw_data.get("summary_details", {})
                    advisory_id = summary.get("notification_id")
                    if not advisory_id: continue

                    # 1. Advisory Record
                    advisories[advisory_id] = (
                        advisory_id, vendor_id, f"VMware Security Advisory {advisory_id}",
                        summary.get("severity"),
                        parse_date(summary.get("initial_publication_date")),
                        parse_date(summary.get("last_updated")),
                        raw_data.get("url")
                    )
                    
                    cve_ids = find_cves_in_text(raw_data)
                    
                    # Handle case with no CVEs
                    if not cve_ids:
                        cve_ids = [f"NOCVE-{advisory_id}"]
                    ...
                    for cve_id in cve_ids:
                        ...
                        cve_product_maps[cve_key] = (
                            vendor_id, cve_id, None, "See advisory URL for patching details."
                        )
...
'''
# VMWare_normal.py (Production Version)
import os
import re
import logging
import json
