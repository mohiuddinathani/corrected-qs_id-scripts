# Openssl_normal.py (Final Production Version)
import os
import re
import logging
import psycopg2
...
logger = logging.getLogger("openssl_normalizer")
load_dotenv()
...
def generate_advisory_id(cve_id: str, staging_id: int) -> str:
    return f"OpenSSL-{cve_id}" if cve_id else f"OpenSSL-ADV-{staging_id}"
        ...
        with psycopg2.connect(**DB_CONFIG) as conn:
            with conn.cursor() as cur:
                ...
                for staging_id, raw_data in tqdm(rows, desc=f"Parsing {VENDOR_NAME} Data"):
                    ...
                    cve_id = raw_data.get("cveMetadata", {}).get("cveId")
                    if not cve_id: continue
                    
                    advisory_id = generate_advisory_id(cve_id, staging_id)
                    title = cna.get("title")
                    description = next((d["value"] for d in cna.get("descriptions", []) if d.get("lang") == "en"), None)
                    severity = next((m.get("other", {}).get("content", {}).get("text") for m in cna.get("metrics", [])), None)
                    release_date = parse_date(cna.get("datePublic"))
                    advisory_url = raw_data.get("advisory_url")
                    
                    cwe_ids = [desc.get("cweId") for pt in cna.get("problemTypes", []) for desc in pt.get("descriptions", []) if desc.get("cweId")]
                    cwe_id_str = ",".join(cwe_ids) if cwe_ids else None
                    
                    refs = [f'{ref.get("name", "")}: {ref.get("url", "")}'.strip() for ref in cna.get("references", [])]
                    reference_url = " | ".join(filter(None, refs)) or None
                    
                    # 1. Advisory
                    advisories[advisory_id] = (
                        advisory_id, vendor_id, title, None, # severity is NULL
                        release_date, None, advisory_url
                    )
                    ...
                    # 2. CVE Record
                    cves[cve_key] = (
                        vendor_id, cve_id, cwe_id_str, clean_text(description),
                        clean_text(severity), None, None, None, None, reference_url
                    )
                    
                    # 3. Product Map
                    cve_product_maps[cve_key] = (vendor_id, cve_id, None, None) # recommendations NULL
...
'''
# Openssl_normal.py (Production Version)
import os
import re
import logging
import psycopg2
        ...
        with psycopg2.connect(**DB_CONFIG) as conn:
            with conn.cursor() as cur:
                ...
                for staging_id, raw_data in tqdm(rows, desc=f"Parsing {VENDOR_NAME} Data"):
                    ...
                    # Per your request, advisory severity is now NULL
                    advisories[advisory_id] = (
                        advisory_id, vendor_id, title, None,
                        release_date, None, advisory_url
                    )
                    ...
                    # Per your request, recommendations is now NULL
                ...
                if cves:
                    execute_values(cur, """
                        ...
                        ON CONFLICT (vendor_id, cve_id) DO UPDATE SET cwe_id=EXCLUDED.cwe_id, description=EXCLUDED.description, severity=EXCLUDED.severity, reference_url=EXCLUDED.reference_url;
                    """, list(cves.values()))
                if cve_product_maps:
                    execute_values(cur, """
