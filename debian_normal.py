# debian_normal.py (Final Production Version)
import os
import logging
import re
import sys
...
logger = logging.getLogger("debian_normalizer")
load_dotenv()
        ...
        with psycopg2.connect(**DB_CONFIG) as conn:
            with conn.cursor() as cur:
                ...
                for staging_id, raw_data in rows:
                    ...
                    # Handle missing IDs defensively
                    if not advisory_id:
                        advisory_id = f"NOID-DEBIAN-{staging_id}"

                    initial_date = parse_date(s_data.get("issue_date"))
                    ...
                    recommendation = " ".join([f"For the '{fix['distribution']}' distribution, fixed in version {fix['version']}." for fix in s_data.get("fixed_in", [])]) or "See advisory URL for details."

                    for cve_id in s_data.get("cves", []):
                        cve_key = (vendor_id, cve_id)
                        
                        # 2. CVEs
                        cves[cve_key] = (
                            vendor_id, cve_id, 
                            None, s_data.get("description"), # cwe_id is NULL
                            None, None, None, # severity, cvss, vector
                            None, None, # dates
                            raw_data.get("source_url")
                        )
...
'''
# debian_normal.py (Production Version)
import os
import logging
import re
import psycopg2
            ...
            with conn.cursor() as cur:
                ...
                for staging_id, raw_data in rows:
                    ...
                    for cve_id in s_data.get("cves", []):
                        cve_key = (vendor_id, cve_id)
                        cves[cve_key] = (
                            vendor_id, cve_id, None, s_data.get("description"), # cwe_id is NULL
                            None, None, None, None, None, # severity, cvss, vector, dates are NULL
                            raw_data.get("source_url")
                        )
                        cve_product_maps[cve_key] = (vendor_id, cve_id, Json(s_data.get("fixed_in")), recommendation)
                        advisory_cves_map.add((advisory_id, vendor_id, cve_id))
