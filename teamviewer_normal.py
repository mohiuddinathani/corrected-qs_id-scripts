# Teamviewer_normal.py (Final Production Version)
import os
import re
import logging
import psycopg2
...
logger = logging.getLogger("teamviewer_normalizer")
load_dotenv()
...
def parse_date(date_str):
    ...
    # Added '%b %d, %Y' to handle "Jan 28, 2025" format
...
def join_cwe(problem_type):
    """Extracts only CWE-XXX identifiers from a string."""
        ...
        with psycopg2.connect(**DB_CONFIG) as conn:
            with conn.cursor() as cur:
                ...
                for staging_id, raw_data in tqdm(rows, desc=f"Parsing {VENDOR_NAME} Data"):
                    ...
                    bulletin_details = raw_data.get("bulletin_details", {}) or {}
                    
                    # 1. Advisory Record
                    advisories[advisory_id] = (
                        advisory_id, vendor_id, raw_data.get("TITLE"),
                        bulletin_details.get("Priority"),
                        parse_date(bulletin_details.get("Issue Date")),
                        parse_date(bulletin_details.get("Last Update")),
                        raw_data.get("URL")
                    )
                    
                    cve_details = raw_data.get("cve_details", {}) or {}
                    if not cve_details: continue

                    recommendations = (raw_data.get("sections") or {}).get("4. Solutions and mitigations")

                    for cve_id in cve_details.get("cve_ids", []):
                        cve_key = (vendor_id, cve_id)
                        
                        # 2. CVE Record
                        cves[cve_key] = (
                            vendor_id, cve_id,
                            join_cwe(cve_details.get("problem_type")),
                            clean_text(cve_details.get("description")),
                            bulletin_details.get("Priority"),
                            safe_numeric(cve_details.get("cvss_score")),
                            cve_details.get("cvss_vector"),
...
'''
# Teamviewer_normal.py (Production Version)
import os
import re
import logging
import psycopg2
