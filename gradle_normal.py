# Gradle_normal.py (Final Production Version)
import os
import re
import logging
import sys
...
logger = logging.getLogger("gradle_normalizer")
load_dotenv()
        ...
        with psycopg2.connect(**DB_CONFIG) as conn:
            with conn.cursor() as cur:
                ...
                for staging_id, raw_data in tqdm(rows, desc=f"Parsing {VENDOR_NAME} Data"):
                    ...
                    if not ref_links and other_info:
                    ...
                    # CWE ID logic
                    cwe_id_str = None
                    if cwe_data := cve_details.get("CWE_Data"):
                        cwe_id_str = ",".join([cwe.get("CWE_ID") for cwe in cwe_data if cwe.get("CWE_ID")])
                    elif cwes_list := cve_details.get("CWEs"):
                        cwe_id_str = ",".join(cwes_list)
                    elif weaknesses_str := sidebar.get("Weaknesses"):
                        cwe_id_str = weaknesses_str
                    
                    # 2. CVE
                    cves[(vendor_id, cve_id)] = (
                        vendor_id, cve_id, cwe_id_str,
                        clean_text(cve_details.get("Description")) or clean_text(cve_details.get("Impact")),
                        sidebar.get("Severity"),
                        sidebar.get("CVSS Score"),
                        sidebar.get("CVSS Vector"),
...
'''
# Gradle_normal.py (Production Version)
import os
import re
import logging
import psycopg2
        ...
        with psycopg2.connect(**DB_CONFIG) as conn:
            with conn.cursor() as cur:
                ...
                for staging_id, raw_data in tqdm(rows, desc=f"Parsing {VENDOR_NAME} Data"):
                    ...
                    # Advisory Record - severity and latest_update_date are NULL per your rule
                    advisories[advisory_id] = (
                        advisory_id,
                        vendor_id,
                        raw_data.get("Title"),
                    ...
                    # CWE ID logic: checks multiple locations
                    cwe_id_str = None
                    if cwe_data := cve_details.get("CWE_Data"):
                        cwe_id_str = ",".join([cwe.get("CWE_ID") for cwe in cwe_data if cwe.get("CWE_ID")])
                    elif cwes_list := cve_details.get("CWEs"):
                    ...
                    # CVE Record - initial_release_date and latest_update_date are NULL per your rule
                    cves[(vendor_id, cve_id)] = (
                        vendor_id,
                        cve_id,
                        cwe_id_str,
                ...
                if cves:
                    execute_values(cur, """
                        ...
                        ON CONFLICT (vendor_id, cve_id) DO UPDATE SET cwe_id=EXCLUDED.cwe_id, description=EXCLUDED.description, severity=EXCLUDED.severity, cvss_score=EXCLUDED.cvss_score, cvss_vector=EXCLUDED.cvss_vector, reference_url=EXCLUDED.reference_url;
                    """, list(cves.values()))
                if cve_product_maps:
                    execute_values(cur, """
